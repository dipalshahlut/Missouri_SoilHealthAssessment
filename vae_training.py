...
# vae_training.py
"""
Stage 2 — VAE Training & Latent Representations (original-behavior clone)

Inputs (read from OUTPUT_DIR):
  - data_scaled.npy
  - prepared_df.parquet (optional; only used to save z_mean_row_keys.npy if aligned)

Outputs (written to OUTPUT_DIR):
  - vae_model.pt               : trained VAE weights (PyTorch state_dict)
  - z_mean.npy                 : latent representations (μ, aligned to data_scaled rows)
  - z_mean_row_keys.npy        : (optional) if prepared_df aligns and has 'mukey'
  - vae_training_loss.png      : training loss curve (matches original filename)
  - train_losses.csv           : (extra) epoch-wise average loss (not used by clustering)
...
try:
    ...
    torch = None
    _torch_import_error = e  # noqa: F841
...
if torch is not None:
    torch.manual_seed(SEED)
    # Keep behavior close to original (no cudnn determinism toggles)

# ---------------- I/O helpers ----------------
def _ensure_outdir(output_dir: Path):
    output_dir.mkdir(parents=True, exist_ok=True)

def _maybe_save_row_keys(output_dir: Path, nrows: int):
    p = output_dir / "prepared_df.parquet"
    if not p.exists():
        return
    try:
        df = pd.read_parquet(p)
        if len(df) == nrows and "mukey" in df.columns:
            keys = df["mukey"].astype(str).to_numpy()
            np.save(output_dir / "z_mean_row_keys.npy", keys)
    except Exception as e:
        logging.warning("Could not read prepared_df.parquet: %s", e)
...
class VAE(nn.Module):
    ...
    def decode(self, z):
        ...
        return self.fc5(h4)  # no final activation
...
def vae_loss_function(x_recon, x, mu, logvar):
    # EXACTLY like original: sum reductions (per-batch ELBO)
    recon_loss = nn.functional.mse_loss(x_recon, x, reduction='sum')
    kl_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    return recon_loss + kl_loss

# ---------------- Core training (original behavior) ----------------
def train_vae_original(X: np.ndarray, *, latent_dim: int, hidden_dim1: int = 64, hidden_dim2: int = 32,
                       epochs: int = 100, batch_size: int = 64, lr: float = 1e-3):
    if torch is None:
        raise ImportError(f"PyTorch required. Original import error: {_torch_import_error}")

    # Keep everything on CPU as in the original snippet (no .to(device))
    ...
    for epoch in range(epochs):
        ...
        for (xb,) in dataloader:
            ...
            # ORIGINAL summed losses
            recon_loss = nn.functional.mse_loss(x_recon, xb, reduction='sum')
            kl_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
            loss = recon_loss + kl_loss

            if torch.isnan(loss):
                logging.error(f"NaN loss encountered at Epoch {epoch+1}. Stopping.")
                sys.exit(1)
...
def compute_z_mean_original(model: VAE, X: np.ndarray) -> np.ndarray:
    # EXACTLY like original: single full encode pass, no batching
    model.eval()
    with torch.no_grad():
        data_tensor = torch.from_numpy(X).float()
    ...
    if np.isnan(z_mean).any():
        logging.error("NaN values found in z_mean.")
        sys.exit(1)
    return z_mean

def plot_loss_original(losses, out_path: Path):
    try:
        import matplotlib.pyplot as plt
        plt.figure(figsize=(8, 5))
        plt.plot(range(1, len(losses) + 1), losses, marker='o', linestyle='-')
        plt.xlabel('Epoch'); plt.ylabel('Average VAE Loss'); plt.title('VAE Training Loss Curve')
        plt.grid(True)
...
def train_and_save(output_dir: Path, latent_dim: int,
                   *, hidden_dim1: int = 64, hidden_dim2: int = 32,
                   epochs: int = 100, batch_size: int = 64, lr: float = 1e-3):
    """
    Backward-compatible entrypoint used by sha_pipeline_runbook.py.
    Trains with original behavior and writes artifacts that downstream expects.
    """
    ...
    # Optional mukey order
    _maybe_save_row_keys(output_dir, X.shape[0])

    # Train (original)
    model, train_losses = train_vae_original(
        X,
        latent_dim=latent_dim,
        hidden_dim1=hidden_dim1,
        hidden_dim2=hidden_dim2,
        epochs=epochs,
        batch_size=batch_size,   # default 64 as original
        lr=lr,                   # default 1e-3 as original
    )

    # Save loss plot (exact filename as original)
    loss_plot_path = output_dir / "vae_training_loss.png"
    plot_loss_original(train_losses, loss_plot_path)

    # Save CSV (extra — harmless)
    try:
        with open(output_dir / "train_losses.csv", "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["epoch", "avg_loss"])
            for i, v in enumerate(train_losses, 1):
                w.writerow([i, float(v)])
    except Exception as e:
        logging.warning("Could not write train_losses.csv: %s", e)

    # Extract latents (original)
    z_mean = compute_z_mean_original(model, X)
    np.save(output_dir / "z_mean.npy", z_mean)

    # Save model weights (extra — harmless)
    try:
        torch.save(model.state_dict(), output_dir / "vae_model.pt")
    except Exception as e:
        logging.warning("Could not save vae_model.pt: %s", e)

    return model, z_mean

def run(output_dir: Path, latent_dim: int, hidden_dim1: int, hidden_dim2: int,
        epochs: int, batch_size: int, lr: float) -> None:
    """
    CLI entry — mirrors original behavior.
    """
    output_dir = Path(output_dir)
    _ensure_outdir(output_dir)

    logging.info(f"=== Stage 2 (original-like) — output_dir={output_dir} ===")
    ...
    logging.info(f"--- 2. Building VAE (Latent Dim = {latent_dim}) ---")
    model, train_losses = train_vae_original(
        X,
        latent_dim=latent_dim,
        hidden_dim1=hidden_dim1,
    ...
    # Plot loss with original name/location
    plot_loss_original(train_losses, output_dir / "vae_training_loss.png")

    # Extract z_mean (original path)
    logging.info(f"--- 4. Extracting Latent Representations (Dim = {latent_dim}) ---")
    z_mean = compute_z_mean_original(model, X)
    np.save(output_dir / "z_mean.npy", z_mean)
    logging.info("Saved z_mean.npy (shape=%s)", z_mean.shape)
    ...
    logging.info("Stage 2 complete (original-behavior).")

# ---------------- CLI ----------------
def parse_args() -> argparse.Namespace:
    default_output = Path(__file__).resolve().parent / "data" / "aggResult"
    p = argparse.ArgumentParser(description="VAE training (original behavior clone)")
    p.add_argument("--output-dir", type=Path, default=default_output)
    p.add_argument("--latent-dim", type=int, default=2)
    p.add_argument("--hidden-dim1", type=int, default=64)
    p.add_argument("--hidden-dim2", type=int, default=32)
